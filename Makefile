...
ARTIFACT_NAME = external-dns-dreamhost-webhook

REGISTRY ?= docker.io/asymingt
IMAGE_NAME ?= external-dns-dreamhost-webhook
IMAGE_TAG ?= latest
...
run:build ## Run the binary on local machine
	build/bin/external-dns-dreamhost-webhook
...
release-check: ## Check if the release will work
	GITHUB_SERVER_URL=github.com GITHUB_REPOSITORY=asymingt/external-dns-dreamost-webhook REGISTRY=$(REGISTRY) IMAGE_NAME=$(IMAGE_NAME) goreleaser release --snapshot --clean --skip=publish
